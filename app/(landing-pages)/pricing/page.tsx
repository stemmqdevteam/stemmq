...
/* ═══════════════════════════════════════════════════
   FAQ — updated to match the 3-tier plan structure
═══════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════
   COMPARISON TABLE — 3 tiers: Free / Pro / Enterprise
═══════════════════════════════════════════════════ */
...
function CellValue({ value }: { value: CellVal }) {
  ...
  if (value === "no") return (
    <div className="flex justify-center">
      <Minus className="h-4 w-4 text-white/14" />
    </div>
  );
  ...
  return <span className="text-[11px] font-semibold text-white/65 block text-center">{value}</span>;
}
...
function PricingHero() {
  return (
    <section className="relative overflow-hidden pt-28 sm:pt-32 pb-2 bg-[#030712]">
      ...
      <div className="relative mx-auto max-w-3xl px-4 sm:px-6 text-center">
        <Reveal>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight text-white leading-[1.08] mb-4">
        ...
        <Reveal delay={0.08}>
          <p className="text-sm sm:text-base md:text-lg text-white/40 max-w-xl mx-auto leading-relaxed">
...
function ComparisonTable() {
  ...
  return (
    <section className="py-16 sm:py-20 bg-[#030712] border-t border-white/5">
      <div className="mx-auto max-w-4xl px-4 sm:px-6">
        <Reveal className="text-center mb-10 sm:mb-12">
          <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight mb-2">
            Compare all features
          </h2>
          <p className="text-sm text-white/35">Everything side by side so you can choose with confidence.</p>
        </Reveal>

        <div ref={ref} className="overflow-x-auto rounded-xl border border-white/8 bg-white/[0.02]">
          <table className="w-full min-w-[500px]">
            <thead>
              <tr className="border-b border-white/8">
                <th className="text-left py-4 px-4 sm:px-5 text-xs font-medium text-white/25 w-[44%]" />
                {tableCols.map((col) => (
                  <th key={col.key}
                    className={`py-4 px-3 sm:px-5 text-center ${col.highlight ? "bg-[#6366f1]/7" : ""}`}>
                    <span className={`text-xs font-bold ${col.highlight ? "text-indigo-300" : "text-white/55"}`}>
                      {col.label}
                    </span>
                  </th>
                ))}
            ...
            <tbody>
              {comparisonCategories.map((cat, catIdx) => (
                <>
                  <tr key={cat.label + "-h"} className={catIdx > 0 ? "border-t border-white/5" : ""}>
                    <td colSpan={4} className="pt-5 pb-2 px-4 sm:px-5">
                      <span className="text-[9px] font-bold uppercase tracking-widest text-white/28">{cat.label}</span>
                    </td>
                  </tr>
                  {cat.rows.map((row, ri) => (
                    <motion.tr key={row.feature}
                      ...
                      className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.016] transition-colors"
                    >
                      <td className="py-3 px-4 sm:px-5 text-[11px] sm:text-xs text-white/50">{row.feature}</td>
        ...
        {/* Legend */}
        <div className="mt-4 flex flex-wrap items-center gap-4 text-[10px] text-white/25 px-1">
          ...
          <div className="flex items-center gap-1.5">
            <Minus className="h-3.5 w-3.5 text-white/14" /> Not included
          </div>
        </div>
      </div>
    </section>
...
function TrustTestimonial() {
  return (
    <section className="py-12 sm:py-14 border-y border-white/5 bg-white/[0.012]">
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        <Reveal>
          <div className="relative rounded-2xl border border-white/10 bg-gradient-to-br from-indigo-500/8 to-violet-500/4 p-6 sm:p-8 overflow-hidden">
            ...
            <Quote className="h-6 w-6 text-indigo-400/25 mb-3" />
            <blockquote className="text-sm sm:text-base md:text-lg text-white/75 leading-relaxed mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/35">CTO · Series C Enterprise SaaS</p>
              </div>
              <span className="ml-auto text-[10px] font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/18 px-2.5 py-1 rounded-full">
                ROI in 60 days
              </span>
            </div>
          </div>
    ...
    <Reveal delay={i * 0.04}>
      <div onClick={() => setOpen(!open)}
        className={`rounded-xl border cursor-pointer overflow-hidden transition-all ${
          open ? "border-indigo-500/25 bg-indigo-500/[0.04]" : "border-white/7 bg-white/[0.02] hover:border-white/12"
        }`}>
        <div className="flex items-center justify-between gap-4 px-4 sm:px-5 py-4">
          <h3 className="text-xs sm:text-sm font-semibold text-white/80 leading-snug">{item.q}</h3>
          <motion.div animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.22 }} className="flex-shrink-0">
            <ChevronDown className={`h-4 w-4 ${open ? "text-indigo-400" : "text-white/25"}`} />
          </motion.div>
        </div>
        <AnimatePresence>
          {open && (
            ...
            >
              <div className="px-4 sm:px-5 pb-4">
                ...
                <p className="text-xs sm:text-sm text-white/45 leading-relaxed">{item.a}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
...
function BottomCTA() {
  return (
    <section className="py-14 sm:py-16 border-t border-white/5 bg-[#030712] relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-2xl px-4 sm:px-6 text-center">
        <Reveal>
          <div className="rounded-2xl border border-indigo-500/18 bg-gradient-to-br from-indigo-500/7 to-violet-500/4 p-8 sm:p-10">
            ...
            <p className="text-sm text-white/40 max-w-sm mx-auto mb-7">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              ...
              <a href="/contact">
                <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                  className="inline-flex items-center gap-2 px-7 py-3 rounded-xl text-sm font-semibold text-white/55 border border-white/10 bg-white/4 hover:bg-white/8 hover:text-white transition-all w-full sm:w-auto justify-center">
                  Talk to sales
                </motion.button>
              </a>
            </div>
            <p className="mt-4 text-[11px] text-white/20">Free plan available · No credit card required</p>
          </div>
        </Reveal>
      </div>
    </section>
