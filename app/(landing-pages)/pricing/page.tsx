  ...
  return (
    <section className="relative overflow-hidden pt-28 sm:pt-32 pb-10 sm:pb-12 bg-[#030712]">
      ...
      <div className="relative mx-auto max-w-4xl px-4 sm:px-6 text-center">
        ...
        {/* Billing toggle */}
        <Reveal delay={0.2}>
          <div className="inline-flex items-center gap-1 rounded-xl border border-white/10 bg-white/4 p-1 mb-2">
            {(["monthly", "yearly"] as const).map((b) => (
              <button
                key={b}
                onClick={() => setBilling(b)}
                className={`relative px-5 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all ${
          ...
          {billing === "yearly" && (
            <motion.p
              ...
              className="text-[11px] text-emerald-400 font-semibold mt-2"
            >
...
function CustomPricingCards({ billing }: { billing: "monthly" | "yearly" }) {
  return (
    <section className="relative pb-16 sm:pb-20 bg-[#030712]">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
            ...
            return (
              <Reveal key={plan.id} delay={i * 0.1}>
                <motion.div
                  ...
                  style={{
                    background: isPro
                      ? "linear-gradient(160deg, rgba(99,102,241,0.12) 0%, rgba(168,85,247,0.06) 100%)"
                ...
                >
                  ...
                  {plan.badge && (
                    <div className="absolute top-0 inset-x-0 flex justify-center">
                      <div
                        className="px-4 py-1 text-[10px] font-bold text-white uppercase tracking-widest rounded-b-xl"
                  ...
                  <div className={`relative flex flex-col flex-1 p-5 sm:p-6 ${plan.badge ? "pt-8" : ""}`}>
                    {/* Icon + Name */}
                    ...
                    <div className="mb-4">
                      {price === null ? (
                        <div>
                          ...
                          <p className="text-xs text-white/35 mt-0.5">Volume-based pricing</p>
                        </div>
                      ) : price === 0 ? (
                        <div>
                          <p className="text-2xl sm:text-3xl font-bold text-white">Free</p>
                          <p className="text-xs text-white/35 mt-0.5">No credit card required</p>
                        </div>
                      ) : (
                        <div className="flex items-end gap-1.5">
                          ...
                          <span className="text-xs text-white/35 mb-1">/ seat / mo</span>
                        </div>
                      )}
                    </div>
                    ...
                    <ul className="space-y-2.5 flex-1 mb-6">
                      {plan.highlights.map((h) => (
                        <li key={h} className="flex items-start gap-2.5">
                          <div
                            className="h-4 w-4 rounded-full flex items-center justify-center mt-0.5 flex-shrink-0"
                            style={{ background: `${plan.color}15`, border: `1px solid ${plan.color}30` }}
                          >
                            <CheckCircle className="h-2.5 w-2.5" style={{ color: plan.color }} />
                          </div>
                    ...
                    {/* CTA */}
                    <Link href={plan.id === "enterprise" ? "/contact" : ROUTES.auth}>
                      <motion.button
                        whileHover={{ scale: 1.03 }}
                        whileTap={{ scale: 0.97 }}
                        className={`relative w-full py-3 rounded-xl text-sm font-semibold transition-all overflow-hidden group ${
                          isPro
        ...
        {/* Enterprise note */}
        <Reveal delay={0.35} className="text-center mt-6">
