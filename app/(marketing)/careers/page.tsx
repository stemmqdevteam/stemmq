...
import {
  ...
  ArrowRight, MapPin, Briefcase, Sparkles, Users, Zap,
  Brain, Shield, TrendingUp, CheckCircle2, ChevronDown,
  Star, Building2, Code2, Megaphone, Settings2
} from "lucide-react";
...
const benefits = [
  { icon: Globe, color: "#6366f1", title: "Remote-first", desc: "Work from anywhere. Async by default, synchronous when it matters." },
  { icon: DollarSign, color: "#10b981", title: "Equity for all", desc: "Every full-time hire receives meaningful equity. We succeed together." },
  { icon: BookOpen, color: "#a855f7", title: "$3,000 learning budget", desc: "Annual stipend for courses, conferences, books, and tools." },
  { icon: Heart, color: "#ef4444", title: "Full healthcare", desc: "Medical, dental, and vision covered 100% for you and dependents." },
  { icon: Clock, color: "#f59e0b", title: "Flexible hours", desc: "Own your schedule. We measure output, not hours online." },
  { icon: Target, color: "#3b82f6", title: "Mission-driven", desc: "We're building infrastructure that changes how organizations think." },
];
...
const openRoles = [
  {
    ...
    color: "#6366f1",
    roles: [
      { title: "Staff Software Engineer — Decision Engine", location: "Remote", type: "Full-time", hot: true },
      { title: "Senior Backend Engineer — Agent Infrastructure", location: "Remote", type: "Full-time", hot: false },
      { title: "ML Engineer — Pattern Recognition", location: "Remote", type: "Full-time", hot: true },
    ],
  },
  {
    dept: "Product",
    ...
    color: "#a855f7",
    roles: [
      { title: "Senior Product Manager — AI Governance", location: "Remote", type: "Full-time", hot: true },
      { title: "Product Designer — Decision Interfaces", location: "Remote", type: "Full-time", hot: false },
    ],
  },
  {
    dept: "Go-to-Market",
    ...
    color: "#10b981",
    roles: [
      { title: "Enterprise Account Executive", location: "Remote / NYC", type: "Full-time", hot: false },
      { title: "Solutions Engineer — Enterprise", location: "Remote", type: "Full-time", hot: false },
      { title: "Content Marketing Manager", location: "Remote", type: "Full-time", hot: false },
    ],
  },
  {
    dept: "Operations",
    ...
    color: "#f59e0b",
    roles: [
      { title: "Legal Counsel — Privacy & AI Compliance", location: "Remote / SF", type: "Full-time", hot: false },
    ],
  },
];

const interviewSteps = [
  { step: "01", title: "Application Review", desc: "We review within 5 business days. No automated screeners. A real human reads every application.", color: "#6366f1" },
  { step: "02", title: "Async Screen", desc: "30-minute async video or written response. No scheduling friction. Answer when you're at your best.", color: "#8b5cf6" },
  { step: "03", title: "Role Interview", desc: "Deep-dive on skills and approach with your direct team. Structured, transparent rubric.", color: "#a855f7" },
  { step: "04", title: "Team Interview", desc: "Meet 2–3 cross-functional team members. Culture and values — it goes both ways.", color: "#3b82f6" },
  { step: "05", title: "Offer", desc: "Transparent offer with full equity and compensation breakdown. No surprises.", color: "#10b981" },
];

const teamValues = [
  { icon: Brain, title: "Curious by default", desc: "We question assumptions — including our own." },
  { icon: Shield, title: "Ownership over credit", desc: "We take responsibility for outcomes, not just tasks." },
  { icon: Zap, title: "Bias toward action", desc: "We ship, learn, and iterate. Fast." },
  { icon: TrendingUp, title: "Slope over position", desc: "We hire for trajectory, not just current ability." },
];
...
function CareersHero() {
  ...
  return (
    <section ref={ref} className="relative overflow-hidden pt-28 sm:pt-32 pb-16 sm:pb-20 bg-[#030712]">
      ...
      <Orb delay={0} className="absolute top-[-8%] left-[-3%] w-[480px] h-[480px] rounded-full bg-indigo-600/10 blur-[120px] pointer-events-none" />
      <Orb delay={4} className="absolute bottom-[-5%] right-[-3%] w-[380px] h-[380px] rounded-full bg-violet-600/8 blur-[90px] pointer-events-none" />
      ...
      <motion.div style={{ y, opacity }} className="relative mx-auto max-w-5xl px-4 sm:px-6 text-center">
        ...
        <Reveal delay={0.14}>
          <p className="text-sm sm:text-base md:text-lg text-white/45 max-w-xl mx-auto mb-10 leading-relaxed">
            A small, focused team building infrastructure that changes how organizations think and decide.
          ...
          <div className="inline-flex items-center gap-0 rounded-2xl border border-white/8 bg-white/3 overflow-hidden mb-10">
            {[
              ...
              <div key={s.label} className={`px-5 sm:px-7 py-3 sm:py-4 text-center ${i < arr.length - 1 ? "border-r border-white/6" : ""}`}>
                <p className="text-lg sm:text-2xl font-bold tabular-nums" style={{ color: s.color }}>{s.value}</p>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <Reveal className="text-center mb-10 sm:mb-14">
          ...
          <p className="mt-3 text-sm sm:text-base text-white/40 max-w-md mx-auto">
            With structure, transparency, and a commitment to getting better over time.
          </p>
        </Reveal>
              ...
              <Reveal key={b.title} delay={i * 0.07}>
                <motion.div whileHover={{ y: -5 }}
                  ...
                  <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"
                    style={{ background: `radial-gradient(circle at 20% 20%, ${b.color}08, transparent 60%)` }} />
                  <div className="relative">
                    <div className="h-10 w-10 sm:h-11 sm:w-11 rounded-2xl flex items-center justify-center mb-4"
                      style={{ background: `${b.color}15`, border: `1px solid ${b.color}25` }}>
                      <Icon className="h-5 w-5" style={{ color: b.color }} />
                    </div>
                    <h3 className="text-sm sm:text-base font-bold text-white mb-2">{b.title}</h3>
                    <p className="text-xs sm:text-sm text-white/50 leading-relaxed">{b.desc}</p>
                  </div>
                </motion.div>
              </Reveal>
            );
        ...
        <Reveal>
          <div className="rounded-2xl border border-white/8 bg-white/2 p-5 sm:p-7">
            <p className="text-[10px] text-white/30 uppercase tracking-widest font-semibold mb-4">Additional Perks</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
              {perks.map((perk, i) => (
                <div key={i} className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs sm:text-sm text-white/55">{perk}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            return (
              <Reveal key={v.title} delay={i * 0.08}>
                <motion.div whileHover={{ y: -5 }}
                  ...
                  <h3 className="text-sm sm:text-base font-bold text-white mb-2">{v.title}</h3>
                  <p className="text-xs sm:text-sm text-white/45 leading-relaxed">{v.desc}</p>
                </motion.div>
              </Reveal>
            );
          })}
...
function OpenRoleCard({ role, color }: { role: (typeof openRoles)[0]["roles"][0]; color: string }) {
  return (
    <motion.div
      whileHover={{ x: 4 }}
    ...
    >
      <div className="flex-1 min-w-0 mr-4">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <p className="text-xs sm:text-sm font-semibold text-white/85 group-hover:text-white transition-colors">{role.title}</p>
          {role.hot && (
...
function OpenRolesSection() {
  ...
  return (
    <section id="roles" className="py-16 sm:py-20 bg-white/[0.015] border-y border-white/5">
      <div className="mx-auto max-w-4xl px-4 sm:px-6">
        <Reveal className="text-center mb-10 sm:mb-12">
          ...
          <p className="mt-3 text-sm text-white/40 max-w-md mx-auto">
            We hire for slope, not just current ability. If you're curious and care deeply about the problem, apply.
          </p>
        </Reveal>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-xl flex items-center justify-center"
                        ...
                        <DIcon className="h-4 w-4" style={{ color: dept.color }} />
                      </div>
                      <div className="text-left">
                        <span className="text-xs sm:text-sm font-bold text-white">{dept.dept}</span>
                    ...
                    <motion.div animate={{ rotate: openDept === dept.dept ? 180 : 0 }} transition={{ duration: 0.25 }}>
                      <ChevronDown className={`h-4 w-4 transition-colors ${openDept === dept.dept ? "text-white/60" : "text-white/25"}`} />
                    </motion.div>
                  </button>
                      ...
                      >
                        <div className="px-3 sm:px-4 pb-3 space-y-2 border-t border-white/6 pt-3">
                          {dept.roles.map((role, ri) => (
                            <OpenRoleCard key={ri} role={role} color={dept.color} />
                          ))}
                        </div>
                      </motion.div>
                    )}
    ...
    <section className="py-16 sm:py-20 bg-[#030712]">
      <div className="mx-auto max-w-4xl px-4 sm:px-6">
        <Reveal className="text-center mb-10 sm:mb-12">
          ...
          <p className="mt-3 text-sm text-white/40 max-w-md mx-auto">
            We respect your time. Our process is async-friendly and designed to give you as much signal as you give us.
          </p>
        </Reveal>
        ...
        <div ref={ref} className="relative max-w-2xl mx-auto">
          ...
          <div className="space-y-3">
            {interviewSteps.map((step, i) => (
              <motion.div
                ...
                style={activeStep === i ? { borderColor: `${step.color}35`, background: `${step.color}05` } : {}}
              >
                ...
                {/* Mobile step badge */}
                <div className="sm:hidden flex h-8 w-8 items-center justify-center rounded-xl text-[10px] font-bold flex-shrink-0 mt-0.5"
                  style={{ background: `${step.color}15`, border: `1px solid ${step.color}25`, color: step.color }}>
                  {step.step}
                </div>

                <div className="flex-1">
                  <p className="text-xs sm:text-sm font-bold text-white mb-1">{step.title}</p>
          ...
          <Reveal delay={0.5}>
            <div className="mt-5 rounded-2xl border border-white/6 bg-white/2 p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-semibold text-white/70 mb-0.5">Typical timeline: 2–3 weeks total</p>
                <p className="text-xs text-white/35">From first application to offer. We move fast and communicate proactively at every step.</p>
              </div>
            </div>
          </Reveal>
        </div>
...
function TeamQuoteSection() {
  const quotes = [
    { text: "I've never worked somewhere that ships this fast while being this thoughtful about quality.", author: "Senior Engineer", tenure: "1.5 years", color: "#6366f1" },
    { text: "The async-first culture is real. I do my best work at 6am and nobody cares what time it is.", author: "Product Designer", tenure: "8 months", color: "#a855f7" },
    { text: "Equity actually vests. The comp is transparent. Everything they promised in the interview is true.", author: "Growth Lead", tenure: "2 years", color: "#10b981" },
  ];
          ...
          {quotes.map((q, i) => (
            <Reveal key={i} delay={i * 0.08}>
              <div className="rounded-2xl border border-white/8 bg-white/2 p-5 sm:p-6 h-full">
                ...
                <p className="text-xs sm:text-sm text-white/65 leading-relaxed mb-5 italic">"{q.text}"</p>
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-white/65">{q.author}</p>
                    <p className="text-[10px] text-white/30">{q.tenure} at StemmQ</p>
                  </div>
                </div>
              </div>
            </Reveal>
